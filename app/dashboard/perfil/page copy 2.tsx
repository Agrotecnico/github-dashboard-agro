...
export default function Page() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="relative w-20 h-20">
        <input
          type="file"
          className="w-20 h-20 cursor-none absolute rounded-full opacity-0"
          onChange= {handleFileChange}
          
        />
        
          {/* <div className="max-h-[80px] max-w-[80px] min-h-[80px] min-w-[80px] min-w-20 relative" data-testid="image-container"> */}
            <button className="cursor-none w-20">
              {session?.user?.image ? (
                <img
                decoding="async" 
      ...
      <div>
        {file && (
          <Image
            ...
            alt="Uploaded file"
            className="w-64 h-64 object-contain mx-auto rounded-full"
            width={80}
            height={80}
          />
        )}
