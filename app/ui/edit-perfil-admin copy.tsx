    ...
    <>
      ...
      <div className="mb-2 mt-6 flex items-start gap-6 min-[375px]:gap-12 ">
        <form onSubmit={uploadToServer } id="subirImage">
          ...
          <div className="mb-0.5 mt-10 text-start text-xs">Cambiar imagen</div>
          <button
            className={`cursor-default disabled:cursor-pointer`}
            disabled= { file && !imageUrl ? ( false
            ) : file && imageUrl ? ( 
              true
            ) : (
              false
          ...
          >
            <Frente
              className={`relative flex w-[164px] flex-col items-center justify-start ${!file && ' opacity-60 '} ${file && !imageUrl && " hover:bg-[#ffffffcc] " }  ${file && imageUrl && "opacity-60 cursor-default " } `}
            >
